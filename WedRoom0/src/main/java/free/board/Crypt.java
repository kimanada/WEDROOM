...
import javax.crypto.Cipher;//암호화 시킬때 필요한 암호객체
    ...
    */
    ...
    private static java.security.Key getKey(String fileurl) throws Exception {
    	if(key==null) {
    		...
    		if(file.exists()) {//파일이 존재한다면 객체직렬화를 통해서 key를 불러옴
    			...
    			throw new Exception("암호 키 객체를 생성할수 없다.");
    		}
    	}
    ...
    public static String encrypt(String ID) throws Exception {
    	...
    	//BASE64Encoder encoder=new BASE64Encoder();//옛날방식
    	Encoder encoder=Base64.getEncoder();
    	//String outputStr1=encoder.encode(outputBytes1);//옛날방식
    ...
    public static String decrypt(String codedID) throws Exception{
    	...
    	//3.암호화할 문자열->byte[]->암호화->byte[]
    	//4.BASE64Encoder클래스->문자열끝을 보완
    	//BASE64Decoder decoder=new BASE64Decoder();//옛날방식
    	Decoder decoder=Base64.getDecoder();
    	
    	//byte[] inputBytes1=decoder.decodeBuffer(codedID);//암호화된 상태 //옛날방식
    	byte[] inputBytes1=decoder.decode(codedID);//암호화된 상태
    ...
    public static void encryptFile(String infile, String outfile) throws Exception {
    	//1.Cipher객체를 생성
    	Cipher cipher=Cipher.getInstance("DES/ECB/PKCS5Padding");//DES/ECB/PKCS5Padding 암호화시키는 알고리즘 계산식
    	//2.Cipher초기화(암호화 옵션,키)
    	cipher.init(Cipher.ENCRYPT_MODE, getKey());//공인인증서
    	//3.암호화을 희망하는 파일명->FileInputStream(원본파일명)
    	//dboracle.properties
    	FileInputStream in=new FileInputStream(infile);//원본 경로포함 파일명
    	FileOutputStream fileOut=
    			           new FileOutputStream(outfile);//새로 암호화된 파일명
    	//4.cipher객체->암호화시킬 파일생성(암호화 시킬 파일객체,cipher객체명)
    	CipherOutputStream out=
    			new CipherOutputStream(fileOut,cipher);
    	//문자열 한라인씩 읽어들여->암호화
    	byte[] buffer=new byte[kBufferSize];//파일의 내용을 담을 객체를 배열로 만듬
    	//원본파일에서 읽어들여->byte[] 담기=>암호화시키는 파일에 담아주는 작업
    	int length;
